...
Future<int> getMaxAvailableTimeSlot() async {
  ...
  int offset = await NTP.getNtpOffset(localeTime: now); // sync time with server
  DateTime syncTime = now.add(Duration(milliseconds: offset));
  //compare syncTime with
  if(syncTime.isAfter(DateTime(now.year, now.month, now.day, 9,0))
